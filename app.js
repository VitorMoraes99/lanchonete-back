...
app.post('/lanches', upload.single('imagem'), (req, res) => {
    ...
    if (!nome || !descricao || !preco || !categoria || !imagePath) {
        res.status(400).json({ message: 'All fields (nome, descricao, preco, categoria, imagem) are required' });
        return;
    }

    db.run(
        'INSERT INTO Lanches (nome, descricao, preco, categoria, imagem) VALUES (?, ?, ?, ?, ?)',
        [nome, descricao, preco, categoria, imagePath],
        function (err) {
            if (err) {
...
app.put('/lanches/:id', (req, res) => {
    ...
    if (!nome || !descricao || !preco || !categoria) {
        res.status(400).json({ message: 'All fields (nome, descricao, preco, categoria) are required' });
        return;
    }

    db.run(
        'UPDATE Lanches SET nome = ?, descricao = ?, preco = ?, categoria = ? WHERE id = ?',
...
app.post('/pedidos', (req, res) => {
    ...
    if (!id_lanche || !quantidade) {
        res.status(400).json({ message: 'All fields (id_lanche, quantidade) are required' });
        return;
    }
